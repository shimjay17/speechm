...
class AlignEvalDataset(torch.utils.data.Dataset[Dict[str, torch.Tensor]]):
    def __init__(self, data_json: Path, tokenizer: PreTrainedTokenizerBase, dataset_name= None) -> None:
        super().__init__()
        ...
        self.examples = []
        for example in examples_dict[dataset_name]:
...
def evaluate(cfg: EvaluationConfig) -> None:
    ...
    for i, batch in enumerate(tqdm(eval_datalodaer_clean, disable=not accel.is_local_main_process)):
        ...
        results.append({
            ...
            "input_length": len(utterance.split()),
