...
from .load_speech import load

from vlm_eval.util.interfaces import SLMEval, SpeechProcessor, Tokenizer


class CobraSLMEval(SLMEval):
    def __init__(
        self,
        model_family: str,
        model_id: str,
        ...
        **_: str,
        ...
        self.generate_kwargs = {"do_sample": True, "use_cache": True, "max_new_tokens": self.max_length, "temperature": self.temperature}

    def load(self) -> Tuple[nn.Module, Tokenizer, SpeechProcessor]:
        ...
        # Get Fully Initialized SLM Instance (+ handle `load_precision`)
        slm = load(load_from, hf_token=self.hf_token)
        slm.to(self.distributed_state.device, dtype=self.dtype)

        # Get Tokenizer and Image Processor
        tokenizer, image_transform = slm.llm_backbone.tokenizer, slm.speech_backbone.speech_augmentation_strategy
        return slm, tokenizer, image_transform
    ...
    def generate_answer(
        ...
        transcription_prompts: List[str],
        return_string_probabilities: Optional[List[str]] = None,
        # use_cache = Optional[bool] = False,
    ) -> Union[List[str], Tuple[List[str], List[List[float]]]]:
        return self.model.generate_batch(
            audio_values, transcription_prompts, return_string_probabilities, **self.generate_kwargs

        )
    
    def generate_answer_no_text(
        self,
        audio_values: torch.Tensor,
        transcription_prompts: List[str],
        return_string_probabilities: Optional[List[str]] = None,
        ...
        return self.model.generate_batch_no_text(
            audio_values, transcription_prompts, return_string_probabilities, **self.generate_kwargs
        )
