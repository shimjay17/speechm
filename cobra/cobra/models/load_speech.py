...
from cobra.models.materialize_speech import get_llm_backbone_and_tokenizer, get_speech_backbone
...
def load(
    model_id_or_path: Union[str, Path], hf_token: Optional[str] = None, cache_dir: Optional[Union[str, Path]] = None, llm_only: bool = False, arch_specifier: Optional[str] = None
):
    ...
    overwatch.info(
        ...
        f"             Speech Backbone =>> [bold]{model_cfg['speech_backbone_id']}[/]\n"
        f"             LLM Backbone    =>> [bold]{model_cfg['llm_backbone_id']}[/]\n"
        f"             Arch Specifier  =>> [bold]{model_cfg['arch_specifier']}[/]\n"
        f"             Checkpoint Path =>> [underline]`{checkpoint_pt}`[/]"
    )

    # Load Speech Backbone
    overwatch.info(f"Loading Speech Backbone [bold]{model_cfg['speech_backbone_id']}[/]")
    speech_backbone, speech_augmentation_strategy = get_speech_backbone(
        model_cfg["speech_backbone_id"],
        model_cfg["speech_backbone_path"],
        model_cfg["speech_augmentation_strategy"],
    )
