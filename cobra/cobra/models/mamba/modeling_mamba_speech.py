...
def decode_no_text(
    ...
    # Ensure input_values is 2D: (batch_size, seq_len)
    if input_values.ndim == 1:
        input_values = input_values.unsqueeze(0)  # Add batch dimension

    if streamer is not None:
        streamer.put(input_values.cpu())

    batch_size, seqlen_og = input_values.shape
    teacher_output_len = teacher_outputs.shape[1] if teacher_outputs is not None else 0
    if use_cache:
        if not hasattr(model, "_decoding_cache"):
            model._decoding_cache = None
    ...
    def get_logits(input_values, inference_params):
        decoding = inference_params.seqlen_offset > 0
        if decoding:
            position_ids = torch.full(
                (batch_size, 1),
        ...
        if not use_cache or not decoding:
            ...
            logits = model._decoding_cache.run(
                input_values, position_ids, inference_params.seqlen_offset
            ).squeeze(dim=1)
        return logits[..., :vocab_size] if vocab_size is not None else logits
    ...
    if enable_timing:
        start.record()
    scores, sequences = [], [input_values]
    if max_new_tokens is not None:
        max_length = sequences[-1].shape[1] + max_new_tokens  # override max_length if max_new_tokens is set
    sequences_cat = input_values
    while not should_stop(sequences[-1], inference_params):
        scores.append(get_logits(sequences[-1], inference_params))
        inference_params.seqlen_offset += sequences[-1].shape[1]
        if repetition_penalty == 1.0:
...
def decode_connector(
    ...
    def get_logits(input_ids, inference_params):
        ...
        if decoding:
            ...
            position_ids = None
        if not use_cache or not decoding:
            # Extract features using speech_backbone
            speech_output = model.speech_backbone(input_values)
            speech_tensor = speech_output.last_hidden_state if hasattr(speech_output, 'last_hidden_state') else speech_output

            # Project features using projector
            projected_output = model.projector(speech_tensor)
            logits = projected_output.squeeze(dim=1)
        else:
            logits = model._decoding_cache.run(
                input_ids, position_ids, inference_params.seqlen_offset
            ).squeeze(dim=1)
...
class GenerationMixin:
    ...
    **kwargs
        ...
        output = decode_no_text(
            input_values, self, max_length=max_length, max_new_tokens=max_new_tokens, top_k=top_k, top_p=top_p, temperature=temperature, **kwargs
        )
        if not output_scores:
            output.scores = None
        return output if return_dict_in_generate else output.sequences
    ...
    def forward(
        ...
        **backbone_kwargs
        ...
        if output_hidden_states:
            return CausalLMOutputWithPast(loss=loss, logits=logits, hidden_states=hidden_states)
