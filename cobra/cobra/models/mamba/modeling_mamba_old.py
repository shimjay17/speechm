...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        # if inference_params is not None:
        #     conv_state, ssm_state = self._get_states_from_cache(inference_params, batch)
        #     # Check if there is a sequence offset and update the state accordingly
        #     if inference_params.seqlen_offset > 0:
        #         # Process each token one at a time if there is a sequence offset
        #         for i in range(seqlen):
        #             # Extract the current token's hidden states across the batch
        #             current_token_hs = hidden_states[:, i:i+1, :]
        #             # Step through the model with the current token's hidden states
        #             out, conv_state, ssm_state = self.step(current_token_hs, conv_state, ssm_state)
        #             # Store the output in the corresponding position of the output_states tensor
        #             output_states[:, i:i+1, :] = out
