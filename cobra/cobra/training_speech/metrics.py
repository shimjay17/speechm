...
class Metrics:
    ...
    def get_status(self, loss: Optional[torch.Tensor] = None) -> str:
        ...
        if self.eval:
            return f"=>> [Global Step] {self.global_step:06d} =>> LR :: {lr:.6f} -- Loss :: {loss:.4f} -- Val Loss :: {self.state['eval_loss']:.4f}"
        else:
            return f"=>> [Global Step] {self.global_step:06d} =>> LR :: {lr:.6f} -- Loss :: {loss:.4f}"

    def commit(
        self, *, global_step: Optional[int] = None, lr: Optional[float] = None, update_step_time: bool = False, eval_loss: Optional[float] = None, **kwargs
    ) -> None:
        ...
        if self.eval and eval_loss is not None:
            self.state["eval_loss"] = eval_loss.item() 
