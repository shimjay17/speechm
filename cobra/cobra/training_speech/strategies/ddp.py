...
class DDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint_and_optimizer(
        self, run_dir: Path, global_step: int, epoch: int, train_loss: Optional[float] = None, only_trainable: bool = True
    ) -> None:
        ...
        # Save Checkpoint, optimizer, and lr scheduler & Copy Latest to `latest-checkpoint.pt`
        torch.save(
            {
    ...
    def resume_from_checkpoint(self, checkpoint_path: Path) -> None:
        """Load a checkpoint from `checkpoint_path` and resume training from that point."""
        assert isinstance(self.vlm, DDP), "resume_from_checkpoint assumes VLM is already wrapped in DDP!"

        # Load Checkpoint
        checkpoint = torch.load(checkpoint_path, map_location=self.device_id)

        # Load Model State Dicts
        for mkey in self.all_module_keys:
            getattr(self.vlm.module, mkey).load_state_dict(checkpoint["model"][mkey])
