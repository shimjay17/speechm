...
class ModelConfig(ChoiceRegistry):
    ...
    speech_backbone_id: str                                 # Pretrained Visual Featurizer (from TIMM) to load
    speech_backbone_path: str
    llm_backbone_id: str                                    # Pretrained LLM (from HF Transformers) to load
    llm_backbone_path: str

    # Backbone Parameters
    speech_augmentation_strategy: str                       # Resizing strategy in < crop | letterbox | corner-pad >
...
class Cobra_3B(ModelConfig):
    ...
    speech_backbone_id: str = "dinosiglip-vit-so-384px"
    llm_backbone_id: str = "mamba-2.8b-zephyr"
    llm_backbone_path: Optional[str] = None

    speech_augmentation_strategy: str = "none"
