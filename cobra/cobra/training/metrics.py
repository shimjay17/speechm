...
class Metrics:
    ...
    def get_status(self, loss: Optional[torch.Tensor] = None) -> str:
        ...
        if self.eval:
            return f"=>> [Global Step] {self.global_step:06d} =>> LR :: {lr:.6f} -- Loss :: {loss:.4f} -- Val Loss :: {self.state['val_loss'][-1]:.4f}"
        else:
    ...
    def commit(
        ...
        if self.eval:
            self.state["eval_loss"].append(kwargs["eval_loss"].detach())
        ...
        if self.eval:
            self.log(
                ...
                metrics={
                    ...
                    f"{prefix}/Validation Loss": self.state["val_loss"][-1],
                },
            )
        else:
            self.log(
